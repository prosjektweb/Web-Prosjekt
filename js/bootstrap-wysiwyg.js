	...
	$.fn.wysiwyg = function(userOptions) {
		var editor = this, selectedRange, options, toolbarBtnSelector, updateToolbar = function() {
			if (options.activeToolbarClass) {
				$(options.toolbarSelector).find(toolbarBtnSelector)
						.each(
								function() {
			...
			$.each(files, function(idx, fileInfo) {
				if (/^image\//.test(fileInfo.type)) {
					alert("fileInfo: " + print_r(fileInfo));
			...
			toolbar.find('input[type=text][data-' + options.commandRole + ']')
					.on(
							...
							function() {
								...
								if (newValue) {
									editor.focus();
									execCommand($(this).data(
											options.commandRole), newValue);
								}
								saveSelection();
							}).on('focus', function() {
						var input = $(this);
